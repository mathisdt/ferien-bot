...
def _calculate_percentage_of_completed_days(data, today, bar_width=25):
    ...
    for i in range(0, len(summer_holidays) - 1):
        if not summer_holidays[i][1] < today < summer_holidays[i + 1][0]:
            continue
        days_completed_in_schoolyear = (today - summer_holidays[i][1]).days - 1
        days_in_schoolyear = (summer_holidays[i + 1][0] - summer_holidays[i][1]).days - 1
...
def get_message(days_to_adjust: int = 0, bar_width: int = 0):
    ...
    school_response = requests.get(f'{config.ferien_api_server}/api/v1/holidays/{config.land_kuerzel}')
    if not school_response:
        raise Exception(f"no data received from {config.ferien_api_server}")
    ...
    for item in school_data:
        ...
        holidays_name = re.sub(r'Beweglich', 'beweglich',
                               re.sub(r'^([^ ]+) [^ ]+ [^ ]+ ?(.*)$', '\\1 \\2', item['name']).strip().title())
        ...
        if days_to_holidays_start <= 0 <= days_to_holidays_end:
            text = f"Yippie! Es sind {holidays_name} in {config.land_name}! Noch {days_to_holidays_end + 1} Tag{'e' if days_to_holidays_end != 0 else ''} schulfrei.\n\n"
            ...
            text = f'Noch {days_to_holidays_start} Tag{"e" if days_to_holidays_start != 1 else ""} bis zu den {holidays_name} in {config.land_name}.\n\n'
            text += _calculate_percentage_of_completed_days(school_data, today, bar_width)
            return text
